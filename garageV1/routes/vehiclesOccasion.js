...
router.get('/', async (req, res) => {
    try {
        // Récupérer les véhicules d'occasion de la base de données
        const [vehicles, fields] = await pool.query("SELECT * FROM vehicles");

        res.render('displayVehiclesOccasion', { vehicles });
    } catch (err) {
        console.error(err);
...
router.post('/edit/:id', upload.single('vehicleImage'), async (req, res) => {
    const vehicleId = req.params.id;
    const { brand, model, year, price, description, mileage } = req.body;
    let image_url = req.body.currentImage;  // Si l'image n'est pas mise à jour, conservez l'ancienne

    if (req.file) {
        image_url = '/uploads/' + req.file.filename;  // Mettez à jour l'URL de l'image si une nouvelle image est fournie
    }
